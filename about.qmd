...
Once inventory data is released the change in inventory is calculated. If the change is positive (inventory increase) or negative (inventory increase) a certain threshold of change must be met which was optimized from the training data. If a change is positive and the threshold is met a short position would be put on and vice versa for a negative change in inventory. Once a position is initiated a stop loss and take profit level is set which was also derived from the training data and a maximum of 2 business days is given before the trade must be closed. Signals would be evaluated each week and if the indicator initiates a trade it will be executed on the close of that day since EIA data comes out in the afternoon and a position would be executed at close.
...
cush_plot <- cush_plot %>% layout(annotations=list(x = 0.01 , y = 1.09, text = "WTI Price vs Cushing Inventory                     3 Month Rolling Correlation", showarrow = F, xref='paper', yref='paper', font = list(size = 20)))
